...
const C = {
  ...
  text: "#e7ecf3", muted: "#94a3b8", amber: "#f59e0b", green: "#10b981",
...
const numInput = {
  ...
  color: C.text, borderRadius: 4, padding: 4,
  textAlign: "center", fontSize: 14, width: "100%",
};
const selectStyle = {
  background: C.panel2, color: C.text, border: `1px solid ${C.border}`,
...
// ─────────────────────────────────────────────────────────────────────────────
// COMPONENTS DEFINED OUTSIDE App — critical: prevents React from remounting
// them when App re-renders, which would wipe their local state.
// ─────────────────────────────────────────────────────────────────────────────

function InfoBlock({ children, warn }) {
  return (
    <div style={{
      background: warn ? "rgba(239,68,68,0.1)" : "rgba(99,102,241,0.1)",
      borderLeft: `3px solid ${warn ? C.red : C.indigo}`,
      padding: "10px 14px", borderRadius: 4, marginBottom: 16,
      fontSize: 13, color: warn ? "#fecaca" : "#cbd5e1", lineHeight: 1.5,
    }}>{children}</div>
  );
}
...
function SectionHeader({ children }) {
  return (
    <div style={{
      ...
      margin: "16px 0 6px", fontWeight: 600, color: "#c7d2fe", fontSize: 14,
    }}>{children}</div>
  );
}
...
function Btn({ children, onClick, green, red, ghost, disabled }) {
  ...
  const fg = ghost ? C.text : (green || red) ? "white" : C.bg;
  return (
...
// MatchRow is outside App so its localA/localB state survives App re-renders.
function MatchRow({ match, pred, result, editable, adminResult, roundState, onSave, onResultSave }) {
  const [localA, setLocalA] = useState(pred?.[0] ?? "");
  const [localB, setLocalB] = useState(pred?.[1] ?? "");
  const [resA, setResA]     = useState(result?.[0] ?? "");
  const [resB, setResB]     = useState(result?.[1] ?? "");
  ...
  async function savePred(side, val) {
    ...
    if (a === "" || b === "") return; // only save when both sides have a value
    try {
      await onSave(match.n, { score_a: Number(a), score_b: Number(b) });
  ...
  if (result && pred?.[0] != null && pred?.[1] != null) {
    ...
    const colors = {
      p8: ["#052e16","#86efac"], p6: ["#1e3a44","#a5f3fc"],
      p5: ["#3a1e44","#e9d5ff"], p1: ["#3f2718","#fbbf24"],
      p0: ["#3f3f46","#a1a1aa"],
    };
    const cls = total === 8 ? "p8" : total === 6 ? "p6" : total === 5 ? "p5" : total === 1 ? "p1" : "p0";
    const [bg, fg] = colors[cls];
    ptsEl = (
      <span style={{ background: bg, color: fg, padding: "2px 6px", borderRadius: 4, fontWeight: 700, fontFamily: "monospace", fontSize: 12 }}>
        +{total}
      </span>
    );
  ...
  const rowBg = (!editable && !adminResult) ? "#0e1429" : C.panel2;
  ...
  return (
    <div style={{
      ...
      <span style={{
        background: C.border, color: "#cbd5e1", padding: "1px 5px",
      ...
      {editable ? (
        ...
        <>
          <input type="number" inputMode="numeric" min={0} max={20}
            ...
            onBlur={(e) => saveResult(0, e.target.value)}
            style={numInput} />
          <span style={{ textAlign: "center", color: C.muted }}>:</span>
          <input type="number" inputMode="numeric" min={0} max={20}
            value={resB}
            onChange={(e) => setResB(e.target.value)}
            onBlur={(e) => saveResult(1, e.target.value)}
            style={numInput} />
        </>
      ) : (
        <span style={{
      ...
      <div style={{ display: "flex", gap: 4, alignItems: "center", justifyContent: "flex-end", minWidth: 80 }}>
        {result != null && (
          <span style={{
            background: "#14532d", color: "#d1fae5", padding: "1px 6px",
            borderRadius: 4, fontWeight: 700, fontFamily: "monospace", fontSize: 12,
...
// AuthView outside App so form inputs don't reset when App re-renders
function AuthView({ roundState, onSuccess }) {
  ...
  const roundLabels = {
    open:   { text: "🟢 Betting round is OPEN",  bg: "rgba(16,185,129,0.18)", color: "#6ee7b7", border: "1px solid #047857" },
    closed: { text: "🔒 Betting round is CLOSED", bg: "rgba(239,68,68,0.18)",  color: "#fca5a5", border: "1px solid #991b1b" },
    idle:   { text: "⏸️ No betting round yet",    bg: "rgba(148,163,184,0.15)", color: "#cbd5e1", border: `1px solid ${C.border}` },
  };
  const pill = roundLabels[roundState] || roundLabels.idle;
      ...
      <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 8, padding: 20 }}>
        ...
        <form onSubmit={submit}>
          ...
          <button type="submit" disabled={loading} style={{
            width: "100%", background: C.amber, color: C.bg, border: 0,
...
export default function App() {
  ...
  const loadGameData = useCallback(async (isAdmin, userId = null) => {
    try {
      ...
      // Fix: load winner pick from leaderboard for the current participant
      if (!isAdmin && userId) {
  ...
  function doLogout() {
    ...
    setUser(null);
    setTab("auth");
    setMyPreds({}); setMyWinner(null);
    setLeaderboard([]); setParticipants([]);
  }
  ...
  function RoundPill() {
    ...
    return (
      <span style={{
        display: "inline-block", padding: "4px 14px", borderRadius: 999,
        fontSize: 13, fontWeight: 600, background: s.bg, color: s.color, border: s.border,
      }}>{s.text}</span>
    );
  }
    ...
    return (
      <div>
        ...
        {config.round_state === "idle" && (
          <InfoBlock warn>⏸️ <b>No betting round is open yet.</b> The admin needs to open a round from the Dashboard before you can enter predictions.</InfoBlock>
        )}
        {config.round_state === "open" && (
          <InfoBlock>
            ✏️ <b>Round is open.</b> Enter your score predictions below — saved automatically when you leave each field.<br />
            <b>Scoring:</b> 5 pts correct direction · +3 exact score · +1 partial · 10 pts tournament winner pick.
          </InfoBlock>
        )}
        ...
        {myEntry && (
          <div style={{
            textAlign: "right", padding: "8px 12px", background: C.panel2,
        ...
        <div style={{
          ...
          padding: 12, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", marginBottom: 16,
        }}>
          ...
          {config.tournament_winner && (
            <span style={{ color: C.muted, fontSize: 13 }}>
              🏆 actual: <b style={{ color: C.amber }}>{withFlag(config.tournament_winner)}</b>
            </span>
          )}
        </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                ...
                <tbody>
                  {leaderboard.map((row, i) => {
                    ...
                    const rowBg = i === 0 ? "rgba(245,158,11,0.15)" : i === 1 ? "rgba(245,158,11,0.08)" : i === 2 ? "rgba(245,158,11,0.04)" : "transparent";
                    let winnerCell;
                    if (winnerKnown) {
                      winnerCell = row.winner_pick
                        ? <>{withFlag(row.winner_pick)}{row.winner_bonus > 0 && <span style={{ color: C.green }}> +10</span>}</>
                        : "—";
  ...
  function ByUser() {
    ...
    return (
      <div>
        ...
        {selected && (
          <div style={{ textAlign: "right", padding: "8px 12px", background: C.panel2, borderRadius: 6, marginBottom: 12, fontSize: 14 }}>
        ...
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 8, padding: 10 }}>
          {matches.map((m) => (
            <MatchRow key={m.n} match={m}
              pred={viewUserPreds[m.n] ?? null}
              result={results[m.n] ?? null}
              editable={false} adminResult={false}
              roundState={config.round_state}
              onSave={() => {}} onResultSave={() => {}}
            />
          ))}
  ...
  function AdminDashboard() {
    ...
    async function setWinner() {
      try {
        const cfg = await api.updateConfig({ tournament_winner: winnerSel || null });
        setConfig(cfg);
    ...
    return (
      <div>
        ...
        <h2 style={{ color: "#fde68a", fontSize: 16, margin: "0 0 8px" }}>Round control</h2>
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 8, padding: 14, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", marginBottom: 20 }}>
          {config.round_state === "idle" && <>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 600, marginBottom: 4 }}>⏸️ Round is idle.</div>
              <div style={{ color: C.muted, fontSize: 12 }}>Open to start collecting predictions from participants.</div>
            </div>
            <Btn green onClick={() => setRoundState("open")}>🟢 Open round</Btn>
          </>}
          {config.round_state === "open" && <>
            <div style={{ flex: 1 }}>
              <div style={{ color: C.green, fontWeight: 600, marginBottom: 4 }}>🟢 Round is OPEN.</div>
              <div style={{ color: C.muted, fontSize: 12 }}>Close to lock bets and reveal them.</div>
            </div>
            <Btn red onClick={() => confirm("Close the round? Bets will be revealed to everyone.") && setRoundState("closed")}>🔒 Close round</Btn>
          </>}
          {config.round_state === "closed" && <>
            <div style={{ flex: 1 }}>
              <div style={{ color: "#fca5a5", fontWeight: 600, marginBottom: 4 }}>🔒 Round is CLOSED.</div>
              <div style={{ color: C.muted, fontSize: 12 }}>Enter results in the Results tab.</div>
            </div>
            <Btn ghost onClick={() => confirm("Reopen? Bets will be hidden again.") && setRoundState("open")}>Reopen</Btn>
          </>}
        </div>

        <h2 style={{ color: "#fde68a", fontSize: 16, margin: "0 0 8px" }}>🏆 Tournament winner</h2>
        ...
        <h2 style={{ color: "#fde68a", fontSize: 16, margin: "0 0 8px" }}>Participants ({participants.length})</h2>
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 8, overflowX: "auto" }}>
  ...
  function AdminResults() {
    ...
    return (
      <div>
        ...
        {config.round_state === "open" && (
          <InfoBlock warn>💡 Round is still open. Close it first so participants can't edit bets after seeing scores.</InfoBlock>
        )}
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 8, padding: 10 }}>
          {matches.map((m) => (
            <MatchRow key={m.n} match={m}
              pred={null} result={results[m.n] ?? null}
              editable={false} adminResult={true}
              roundState={config.round_state}
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", color: C.text, fontFamily: '-apple-system,"Segoe UI",Arial,sans-serif' }}>
      {/* Nav */}
      <nav style={{ background: C.panel, borderBottom: `1px solid ${C.border}`, padding: "12px 0" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 16px", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            {tabs.map((t) => (
              <button key={t.id} onClick={() => setTab(t.id)} style={{
                ...
                color: tab === t.id ? (t.admin ? "white" : C.bg) : (t.admin ? "#fda4af" : C.text),
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", fontSize: 13 }}>
            {user && <span>Hi <b style={{ color: "#fde68a" }}>{user.name}</b>{user.is_admin ? " 👑" : ""}</span>}
            {user && (
              <button onClick={doLogout} style={{
                background: "transparent", border: `1px solid ${C.border}`,
                color: C.text, padding: "5px 10px", borderRadius: 6, cursor: "pointer", fontSize: 12,
              }}>Log out</button>
            )}
          </div>
        </div>
      ...
      {globalErr && (
        <div style={{ background: "#7f1d1d", borderBottom: `1px solid ${C.red}`, padding: "8px 16px", fontSize: 13, color: "#fecaca", display: "flex", justifyContent: "space-between" }}>
          <span>⚠️ {globalErr}</span>
          <button onClick={() => setGlobalErr("")} style={{ background: "none", border: 0, color: "#fca5a5", cursor: "pointer" }}>✕</button>
        </div>
      )}
      ...
      {toast && (
        <div style={{
          ...
          color: toast.kind === "warn" ? C.bg : "white",
          padding: "8px 16px", borderRadius: 6, fontSize: 14,
          zIndex: 100, boxShadow: "0 4px 12px rgba(0,0,0,0.3)", whiteSpace: "nowrap",
        }}>{toast.msg}</div>
      )}
    </div>
  );
